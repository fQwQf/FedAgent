"""
E3: FedPow Alpha-Sweep Experiment

Validates: exists optimal alpha* in (0,1) that balances bias and variance.

  hat_g(alpha) = sum(||g_k||^{-alpha} * g_k) / sum(||g_k||^{-alpha})

  alpha=0   -> FedAvg (uniform weights)
  alpha=0.5 -> FedPow-0.5 (predicted optimal)
  alpha=1.0 -> FedNorm (inverse-norm weights)

Usage:
    python scripts/e3_fedpow_sweep.py --device cuda:4
    python scripts/e3_fedpow_sweep.py --device cuda:4 --alphas 0,0.25,0.5,0.75,1.0
"""
...
def run_fedpow_online(alpha, device, train_tensors, eval_loaders):
    """Online FedPow: success-filter (median loss) + FedPow aggregation."""
    method_name = f"online_fedpow_a{alpha:.2f}"
    print(f"\n{'='*60}")
    print(f"  ONLINE FedPow  alpha={alpha}  |  Rounds: {NUM_ROUNDS}")
    print(f"{'='*60}")
    ...
    for rnd in range(NUM_ROUNDS):
        t0 = time.time()
        round_data = {"round": rnd, "method": method_name, "alpha": alpha, "mode": "online"}
        ...
        if alpha == 0.0:
            ...
            weights = [n ** (-alpha) for n in norms]

        agg_delta = _weighted_average_state_dicts(deltas, weights)

        pk_str = "  ".join(f"{e}={round_data[f'{e}_p_k']:.3f}" for e in ENV_NAMES)
        norm_str = "  ".join(f"{e}={round_data[f'{e}_delta_norm']:.4f}" for e in ENV_NAMES)
        w_str = "  ".join(f"{e}={w:.4f}" for e, w in zip(ENV_NAMES, weights))
        print(f"  Round {rnd}: p_k=[{pk_str}]")
        print(f"          norms=[{norm_str}]")
        print(f"          w(a={alpha})=[{w_str}]")
...
def run_fedpow_offline(alpha, device, train_tensors, eval_loaders):
    """Offline FedPow: all data + FedPow aggregation."""
    method_name = f"offline_fedpow_a{alpha:.2f}"
    print(f"\n{'='*60}")
    print(f"  OFFLINE FedPow  alpha={alpha}  |  Rounds: {NUM_ROUNDS}")
    print(f"{'='*60}")
    ...
    for rnd in range(NUM_ROUNDS):
        t0 = time.time()
        round_data = {"round": rnd, "method": method_name, "alpha": alpha, "mode": "offline"}
        deltas, norms = [], []
...
def main():
    ...
    parser.add_argument("--mode", choices=["online", "offline", "both"], default="both")
    ...
    print("=" * 60)
    print("  E3: FedPow Alpha-Sweep")
    print(f"  Model: {MODEL_NAME}  |  Device: {device}  |  Rounds: {NUM_ROUNDS}")
    print(f"  Alphas: {alphas}")
    print(f"  Formula: hat_g(a) = sum(||g_k||^{{-a}} * g_k) / sum(||g_k||^{{-a}})")
    print("=" * 60)

    print("\n  Loading tokenizer & model for tokenization...")
    ...
    modes = ["online", "offline"] if args.mode == "both" else [args.mode]
    all_results = {}
    for mode in modes:
        for alpha in alphas:
            if mode == "online":
                results = run_fedpow_online(alpha, device, train_tensors, eval_loaders)
                key = f"online_alpha_{alpha:.2f}"
                save_subdir = f"online_alpha_{alpha:.2f}"
            else:
                results = run_fedpow_offline(alpha, device, train_tensors, eval_loaders)
                key = f"offline_alpha_{alpha:.2f}"
                save_subdir = f"alpha_{alpha:.2f}"

            all_results[key] = results

            save_dir = Path(f"outputs/e3_fedpow/{save_subdir}")
    ...
    print(f"\n{'='*80}")
    print("  E3 FINAL COMPARISON: FedPow Alpha-Sweep")
    print(f"{'='*80}")

    print(f"{'Round':>5}", end="")
    for key in all_results:
        a = all_results[key][0]["alpha"]
        print(f" | a={a:.2f} avg>8  std>8", end="")
    print()
    print("-" * 80)
    ...
    for rnd in range(NUM_ROUNDS):
        print(f"{rnd:>5}", end="")
        for key in all_results:
            d = all_results[key][rnd]
            print(f" | {d['avg_eval_loss']:>8.4f} {d['eval_loss_std']:>8.4f}", end="")
        print()

    print(f"\n{'='*80}")
    print("  FINAL ROUND (Round {}) COMPARISON".format(NUM_ROUNDS - 1))
    print(f"{'='*80}")
    print(f"{'Alpha':>8} | {'Avg Loss':>10} | {'Std':>8} | ", end="")
    for env in ENV_NAMES:
        print(f"{env:>10}", end=" ")
    print()
    print("-" * 80)
    ...
    for key in all_results:
        d = all_results[key][-1]
        a = d["alpha"]
        print(f"{a:>8.2f} | {d['avg_eval_loss']:>10.4f} | {d['eval_loss_std']:>8.4f} | ", end="")
        for env in ENV_NAMES:
    ...
    summary = {
        ...
        "final_round": {
            key: {
                "alpha": all_results[key][-1]["alpha"],
                "mode": all_results[key][-1].get("mode", "offline"),
                "avg_eval_loss": all_results[key][-1]["avg_eval_loss"],
                "eval_loss_std": all_results[key][-1]["eval_loss_std"],
                "per_env": {env: all_results[key][-1][f"{env}_eval_loss"] for env in ENV_NAMES},
                "delta_norms": {env: all_results[key][-1][f"{env}_delta_norm"] for env in ENV_NAMES},
            }
            for key in all_results
        },
    }
    with open("outputs/e3_fedpow/summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\n  Summary saved to outputs/e3_fedpow/summary.json")
