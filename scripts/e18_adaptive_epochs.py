"""
E18: Adaptive Local Epochs for Heterogeneous Task FL

Core insight: Easy tasks overfit on small data (64 samples) and produce noisy
gradients. Hard tasks underfit and need more training. Instead of weighting
gradients differently (loss-proportional), control gradient quality at the
source by adaptively setting local epochs per task.

Formula: E_k = clip(base_E * L_k / mean(L), 1, max_E)
  where L_k = eval loss of task k

This prevents easy tasks from overfitting (fewer epochs) while giving hard
tasks more adaptation time (more epochs).

Usage:
    python scripts/e18_adaptive_epochs.py --model_size 0.5B --device cuda:3
"""
...
def train_client_epochs(model, dataloader, device, num_epochs):
    model.train()
...
def compute_adaptive_epochs(eval_losses, base_epochs=3, max_epochs=10):
    mean_loss = np.mean(list(eval_losses.values()))
    epochs = {}
    for env in ALL_ENVS:
        raw = base_epochs * eval_losses[env] / mean_loss
        epochs[env] = int(np.clip(raw, 1, max_epochs))
    return epochs

def run_adaptive_epochs(device, train_dl, eval_dl, num_rounds, model_name):
    print(f"\n{'='*70}")
    print(f"  Adaptive Epochs  (base=3, max=10, {num_rounds}R, {model_name})")
    print(f"{'='*70}")
    ...
    for rnd in range(num_rounds):
        t0 = time.time()
        rd = {"round": rnd, "method": "adaptive_epochs"}
        ...
        adaptive_epochs = compute_adaptive_epochs(eval_losses, base_epochs=3, max_epochs=10)
        for env in ALL_ENVS:
            rd[f"{env}_epochs"] = adaptive_epochs[env]

        # Client updates with adaptive epochs
        deltas = {}
        for env in ALL_ENVS:
            apply_lora_state(model, global_state, device)
            train_client_epochs(model, train_dl[env], device, adaptive_epochs[env])
        ...
        avg_delta = _weighted_average_state_dicts(delta_list, [1.0] * len(delta_list))
        ...
        epoch_str = " ".join(f"{e[:3]}E={adaptive_epochs[e]}" for e in ALL_ENVS)
        print(f"  R{rnd:>2}: {loss_str}  hard={hard_avg:.3f}  |  {epoch_str}")
...
def run_baseline_fixed_epochs(device, train_dl, eval_dl, num_rounds, model_name, fixed_epochs=3):
    print(f"\n{'='*70}")
    print(f"  Baseline Fixed Epochs  (E={fixed_epochs}, {num_rounds}R, {model_name})")
    print(f"{'='*70}")
    ...
    for rnd in range(num_rounds):
        t0 = time.time()
        rd = {"round": rnd, "method": "fixed_epochs"}
        ...
        for env in ALL_ENVS:
            apply_lora_state(model, global_state, device)
            train_client_epochs(model, train_dl[env], device, fixed_epochs)
            new_state = get_lora_state(model)
            deltas[env] = OrderedDict({k: new_state[k].float() - global_state[k].float() for k in global_state})
...
def main():
    ...
    parser.add_argument("--method", default="all", choices=["all", "adaptive", "baseline"])
    ...
    print("=" * 70)
    print(f"  E18: Adaptive Local Epochs")
    ...
    methods = (args.method.split(",") if args.method != "all"
               else ["adaptive", "baseline"])

    all_results = {}
    for m in methods:
        if m == "adaptive":
            all_results[m] = run_adaptive_epochs(device, train_dl, eval_dl, args.rounds, model_name)
        else:
            all_results[m] = run_baseline_fixed_epochs(device, train_dl, eval_dl, args.rounds, model_name, fixed_epochs=3)

        save_dir = Path(f"outputs/e18_adaptive_epochs/{args.model_size}/{m}")
        save_dir.mkdir(parents=True, exist_ok=True)
        with open(save_dir / "metrics.json", "w") as f:
            json.dump(all_results[m], f, indent=2)

    print(f"\n{'='*70}")
    print(f"  E18 RESULTS ({args.model_size})")
    print(f"{'='*70}")
    ...
    if "adaptive" in all_results and "baseline" in all_results:
        a_hard = all_results["adaptive"][-1]["hard_avg"]
        b_hard = all_results["baseline"][-1]["hard_avg"]
        delta = a_hard - b_hard
        pct = delta / b_hard * 100 if b_hard != 0 else 0
        print(f"\n  Adaptive vs Baseline: {delta:+.4f} ({pct:+.1f}%) on hard_avg")

    print(f"\n  Results saved to outputs/e18_adaptive_epochs/{args.model_size}/")
